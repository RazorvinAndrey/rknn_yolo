...
def main():
    ...
    print(f"[INFO] Video {src_w}x{src_h}@{src_fps:.2f} | Model input: {args.inw}x{args.inh} | Backend: {args.backend}")
    ...
    try:
        while True:
            ...
            # --- Preprocess: grayscale -> resize -> float32 [0,1] NCHW ---
            gray = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2GRAY)
            img_rs = cv2.resize(gray, (args.inw, args.inh), interpolation=cv2.INTER_AREA)
            img_nchw = img_rs[None, None, :, :].astype(np.float32) / 255.0
            ...
            if args.show:
                cv2.imshow('SuperPoint', frame_bgr)
