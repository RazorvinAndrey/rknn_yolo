...
def parse_args():
    ap = argparse.ArgumentParser(description='YOLOv5 RKNN video with NPU_CORE_0_1_2 + boxes + FPS')
    ...
    ap.add_argument('--avg', type=int, default=30, help='окно усреднения FPS')
    ...
    ap.add_argument('--anchors', type=str, required=True,
                    help='путь к txt с анкерами YOLOv5 (как anchors_yolov5.txt из model_zoo)')
    return ap.parse_args()

# ----------- функции постпроцесса (ЙОLOv5 как в вашем примере) -----------
...
def box_process(position, anchors):
    # position: (na, 4, H, W) после reshape
...
def post_process(input_data, anchors):
    """
    Ожидаем 3 выхода (по одной карте на масштаб), каждый: [1, 255, H, W].
    Преобразуем: 1*255*H*W -> 3*85*H*W и декодируем, как в вашем примере.
    """
    ...
    for c in set(classes):
        ...
        b = boxes[inds]; c_arr = classes[inds]; s = scores[inds]
        keep = nms_boxes(b, s)
        if len(keep) != 0:
            nboxes.append(b[keep]); nclasses.append(c_arr[keep]); nscores.append(s[keep])
...
def draw(image, boxes, scores, classes):
    for box, score, cl in zip(boxes, scores, classes):
        top, left, right, bottom = [int(_b) for _b in box]  # сохраняю порядок как в вашем примере
...
def main():
    ...
    # загрузим анкеры (как в вашем примере YOLOv5)
    if not os.path.isfile(args.anchors):
        print(f"Anchors file not found: {args.anchors}")
        sys.exit(1)
    with open(args.anchors, 'r') as f:
        values = [float(_v) for _v in f.readlines()]
        anchors = np.array(values).reshape(3, -1, 2).tolist()
    print(f"use anchors from '{args.anchors}', which is {anchors}")
    ...
    try:
        while True:
            ...
            if not args.no_display:
                cv2.imshow('YOLOv5 RKNN (NPU 0_1_2)', frame_to_show)
